...
// Apply a JSON changes array to the stored file, persist, and return updated meta.
// Used by the unit test (JSON path).
export async function applyUpdate(id, jsonBody) {
  ...
  s.applyChanges(JSON.stringify(changes));
...
export async function handleApi(req, res, mode) {
  ...
  if (mode === "serve" && command === "update-file") {
    ...
    const id = (req.url.split("id=")[1] || "").split("&")[0] || process.env.PENCILPOT_FILE_ID;
    const f = readFile(id);
    if (!f) return replayFixture("update-file", res);
    const { createSession } = await import("../../headless-core/target/headless/penpot.js");
    const s = createSession(JSON.stringify({ fromTransit: f.transit, meta: f.meta }));
    s.applyTransitUpdate(body);
    const { meta, transit } = JSON.parse(s.getFileResponse());
    const newRevn = (f.meta.revn ?? 0) + 1;
    meta.revn = newRevn;
    writeFile(id, transit, meta);
    // SPA expects application/transit+json: {"~:revn": N, "~:lagged": []}
    // We emit a minimal transit-encoded map matching the recorded response shape.
    const transitResp = `["^ ","~:revn",${newRevn},"~:lagged",[]]`;
